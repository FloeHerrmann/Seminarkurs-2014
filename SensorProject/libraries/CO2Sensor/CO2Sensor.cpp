...
CO2Sensor :: CO2Sensor(uint8_t Rx,uint8_t Tx)
{
...
int CO2Sensor :: cmdInit()
{
	return sendRequest(3,4,0);
}
double CO2Sensor :: getCO2(char format)
{
...
double CO2Sensor :: getTemp(char unit)
{
...
double CO2Sensor :: getRH()
{
	double RH = sendRequest(2,7,3);
	RH/=100;
	return RH;
}
void CO2Sensor :: chkSensorType()
{
	chkASCII();
	chkK33();
}

void CO2Sensor::chkK33()
{
...
int CO2Sensor :: sendRequest(int reqType, int respSize, int respInd)
{
	long Val=-255;
	int cmdTimeout =0;
...
long CO2Sensor :: getResp(int size, int strt)
{
	byte packet[size];
